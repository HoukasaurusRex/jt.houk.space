...
I need to get the test data in my local db to upload to BigQuery using the data pipeline tool.
...
I'm waiting on the platform team to review and merge my PR in the infra access repo (not urgent).
Thomas from Insights left some good feedback on my PR in the data pipeline tool to make it more inline with what we will need for production so I'll need to address those comments as well.
...
- Setback with the local dev environment to get cash management working locally to generate test data so had to manually insert test data into the local db which is time consuming.
