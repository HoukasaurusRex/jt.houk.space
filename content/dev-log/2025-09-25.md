...
I need to create a new feature flag for the frontend work in the the feature flag service.
Got some feedback from Thomas on merchant insights on my PR in the data pipeline tool so I'll need to address those comments as well.
I'll focus on creating demo data and testing the end-to-end flow of data from the local db to BigQuery using the data pipeline tool.
I might need to open a request in an internal platform channel to get someone to help review and merge my PR in the infra access repo.
...
- Addressed Thomas's comments on my PR in the data pipeline tool and made the requested changes.
- Struggled with the local dev environment to get cash management working locally in order to create local test data.
